...
class Settings(BaseSettings):
    ...
    music_provider: str = Field(
        default="", description="Fournisseur de musique actif: spotify | deezer | local | jarvis"
    )
    music_library_dir: str = Field(
        default="music_library",
        description="Dossier des pistes locales pour le player natif Jarvis (provider 'jarvis').",
    )
    ...
    face_recognition_threshold: float = Field(
        default=0.50,
        description=(
            "Similarité cosinus min pour une correspondance ArcFace "
            "(plus HAUT = plus strict — sens inversé par rapport à l'ancien "
            "backend dlib qui mesurait une distance)."
        ),
    )
