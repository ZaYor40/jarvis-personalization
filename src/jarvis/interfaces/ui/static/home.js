...
(function () {
  ...
  async function fetchMusicStatus() {
    // Le player natif Jarvis pilote son propre état (audio element, événements
    // play/pause/timeupdate) — /api/music/status ne le connaît pas et
    // renverrait "non connecté", écrasant l'affichage en boucle toutes les 5s.
    if (_jarvisActive) return;
    try {
  ...
  // ── Player natif Jarvis (provider "jarvis") ─────────────────────────
  // Contrairement à Spotify (SDK + device distant) et Deezer/local (poll
  // API), la lecture a lieu directement dans cet onglet via <audio> — l'état
  // du widget est dérivé des events de l'élément lui-même, pas d'un poll
  // serveur. _jarvisActive distingue "ce provider pilote le widget" pour
  // que les boutons hcw-prev/play/next lui soient prioritairement routés.
  let _jarvisActive = false;
  let _jarvisQueue   = [];   // liste de filenames (source "local" uniquement)
  let _jarvisIndex   = 0;
  let _jarvisSource  = null; // "local" | "soundcloud"
  let _jarvisTitle   = "";
  let _jarvisArtist  = "";

  function initJarvisPlayer() {
    const audio = document.getElementById("jarvis-player");
    if (!audio) return;
    ...
    function pushWidget() {
      updateMusicWidget({
        ...
        provider: "jarvis",
        track: _jarvisTitle,
        artist: _jarvisArtist,
        is_playing: !audio.paused,
        progress_ms: (audio.currentTime || 0) * 1000,
        duration_ms: (audio.duration || 0) * 1000,
      });
    }

    audio.addEventListener("play",  pushWidget);
    audio.addEventListener("pause", pushWidget);
    audio.addEventListener("timeupdate", pushWidget);
    audio.addEventListener("ended", () => {
      if (_jarvisSource === "local" && _jarvisQueue.length) {
        _jarvisIndex = (_jarvisIndex + 1) % _jarvisQueue.length;
        jarvisLoadLocal(_jarvisQueue[_jarvisIndex]);
      }
    });
    // Sans ça, un flux qui échoue au chargement (mauvaise URL, CORS, 404...)
    // ne déclenche ni "play" ni le .catch() de jarvisTryPlay (play() peut se
    // résoudre avant que l'échec réseau n'arrive) — silence total, widget
    // resté sur son état par défaut. On le rend visible.
    audio.addEventListener("error", () => {
      const err = audio.error;
      console.warn("[jarvis-player] Échec de chargement du flux audio", err && err.code, err && err.message);
      updateMusicWidget({
        connected: true, provider: "jarvis",
        track: "Échec de lecture : " + (_jarvisTitle || "piste inconnue"),
        artist: "", is_playing: false, progress_ms: 0, duration_ms: 0,
      });
    });

    // Périphérique de sortie choisi dans Réglages > Musique (settings.js,
    // document iframe séparé — localStorage est le pont entre les deux
    // documents, même origine). setSinkId absent sur ce navigateur → no-op.
    function applySinkPreference() {
      if (typeof audio.setSinkId !== "function") return;
      const id = localStorage.getItem("jarvis_audio_sink_id") || "";
      audio.setSinkId(id).catch((err) => {
        console.warn("[jarvis-player] setSinkId a échoué", err);
      });
    }
    applySinkPreference();
    // 'storage' se déclenche sur les AUTRES documents same-origin (l'iframe
    // réglages) quand la clé change — pas sur le document qui écrit lui-même.
    window.addEventListener("storage", (e) => {
      if (e.key === "jarvis_audio_sink_id") applySinkPreference();
    });
  }

  // Une lecture déclenchée par un message WebSocket (pas un clic direct) peut
  // être bloquée en silence par la politique anti-autoplay du navigateur —
  // audio.play() rejette sa promesse sans aucune erreur visible à l'écran.
  // On le rend visible (console + widget en pause) : un clic sur ▶ dans le
  // widget est un vrai geste utilisateur et débloque la lecture.
  function jarvisTryPlay(audio) {
    audio.play().catch((err) => {
      console.warn("[jarvis-player] Lecture bloquée par le navigateur (autoplay) — clique sur ▶ dans le widget musique pour démarrer.", err);
      updateMusicWidget({
        connected: true, provider: "jarvis", track: _jarvisTitle, artist: _jarvisArtist,
        is_playing: false, progress_ms: 0, duration_ms: 0,
      });
    });
  }

  function jarvisLoadLocal(filename) {
    const audio = document.getElementById("jarvis-player");
    if (!audio) return;
    _jarvisActive = true;
    _jarvisSource = "local";
    _jarvisTitle  = filename.replace(/\.[^.]+$/, "");
    _jarvisArtist = "";
    updateMusicWidget({
      connected: true, provider: "jarvis", track: _jarvisTitle, artist: "Chargement…",
      is_playing: false, progress_ms: 0, duration_ms: 0,
    });
    audio.src = "/api/player/library/stream/" + encodeURIComponent(filename);
    jarvisTryPlay(audio);
  }

  function jarvisPlayEvent(data) {
    const audio = document.getElementById("jarvis-player");
    if (!audio) return;
    setCtrl("music", true); // ouvre le widget automatiquement — visible sans clic préalable
    _jarvisActive = true;
    _jarvisSource = data.source;
    if (data.source === "local") {
      _jarvisQueue = data.queue || [];
      _jarvisIndex = data.index || 0;
      if (_jarvisQueue.length) jarvisLoadLocal(_jarvisQueue[_jarvisIndex]);
    } else {
      _jarvisQueue = [];
      _jarvisTitle  = data.title  || "";
      _jarvisArtist = data.artist || "";
      updateMusicWidget({
        connected: true, provider: "jarvis", track: _jarvisTitle, artist: "Chargement…",
        is_playing: false, progress_ms: 0, duration_ms: 0,
      });
  ...
  function jarvisControlEvent(action) {
    const audio = document.getElementById("jarvis-player");
    if (!audio || !_jarvisActive) return;
    if (action === "pause") audio.pause();
    if (action === "resume") jarvisTryPlay(audio);
    if (action === "next" && _jarvisQueue.length) {
      _jarvisIndex = (_jarvisIndex + 1) % _jarvisQueue.length;
      jarvisLoadLocal(_jarvisQueue[_jarvisIndex]);
    }
    if (action === "previous" && _jarvisQueue.length) {
      _jarvisIndex = (_jarvisIndex - 1 + _jarvisQueue.length) % _jarvisQueue.length;
      jarvisLoadLocal(_jarvisQueue[_jarvisIndex]);
    }
  }

  // Déclenchement manuel direct — n'attend pas que le modèle appelle
  // réellement player_control (peu fiable avec certains providers/modèles).
  // Un clic ici va droit à /api/player/library puis pilote <audio> comme le
  // ferait un vrai player_play reçu par WebSocket, sans dépendre du LLM.
  async function jarvisShuffleLibrary() {
    const btn = document.getElementById("hcw-shuffle-local");
    if (btn) btn.disabled = true;
    try {
      const tracks = await J.api.get("/api/player/library");
      if (!tracks || !tracks.length) {
        J.notify({ kind: "error", text: "Bibliothèque locale vide — dépose des fichiers dans music_library/." });
        return;
      }
      const queue = tracks.map(t => t.filename);
      for (let i = queue.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [queue[i], queue[j]] = [queue[j], queue[i]];
      }
      jarvisPlayEvent({ source: "local", queue, index: 0 });
    } catch (e) {
      J.notify({ kind: "error", text: "Impossible de lire la bibliothèque locale." });
    } finally {
      if (btn) btn.disabled = false;
    }
  }
  document.getElementById("hcw-shuffle-local")?.addEventListener("click", jarvisShuffleLibrary);

  // Clic sur la barre de progression → saute à la position correspondante.
  // Uniquement pour le player natif (Spotify/Deezer géreraient un seek via
  // leurs propres API, pas branché ici — cf. discussion produit).
  document.getElementById("hcw-progress-wrap")?.addEventListener("click", (e) => {
    if (!_jarvisActive) return;
    const audio = document.getElementById("jarvis-player");
  ...
  // Boutons du widget musique — player natif Jarvis en priorité (si actif),
  // sinon SDK Spotify, sinon API générique (Deezer/local en fallback).
  document.getElementById("hcw-prev")?.addEventListener("click", () => {
    if (_jarvisActive) { jarvisControlEvent("previous"); return; }
  ...
  document.getElementById("hcw-play")?.addEventListener("click", () => {
    if (_jarvisActive) { jarvisControlEvent(_musicPlaying ? "pause" : "resume"); return; }
  ...
  document.getElementById("hcw-next")?.addEventListener("click", () => {
    if (_jarvisActive) { jarvisControlEvent("next"); return; }
  ...
  function connectWS() {
    ...
    _ws.onmessage = (ev) => {
      ...
      // ── Player natif Jarvis ─────────────────────────────────────────
      if (data.type === "player_play")    jarvisPlayEvent(data);
      if (data.type === "player_control") jarvisControlEvent(data.action);
