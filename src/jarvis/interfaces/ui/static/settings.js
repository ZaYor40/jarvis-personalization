...
(function () {
  ...
  // Même rendu visuel que makeSelect (classes .csel-*), mais pour une
  // préférence purement locale au navigateur : onSave(value, label) est
  // appelé au clic sur "Sauv." au lieu de POST /api/settings/update.
  function makeLocalSelect(options, currentValue, onSave) {
    let selected = options.some(o => o.value === currentValue) ? currentValue : (options[0]?.value ?? "");
    const labelFor = v => (options.find(o => o.value === v)?.label) || "—";

    const wrap = el("div", { class: "csel-wrap" });
    const btn  = el("button", { class: "csel-btn" });
    const lbl  = el("span", { class: "csel-label", text: labelFor(selected) });
    const chev = el("span", { class: "csel-chevron",
      html: '<svg width="10" height="6" viewBox="0 0 10 6" fill="none"><path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>' });
    btn.appendChild(lbl); btn.appendChild(chev);
    wrap.appendChild(btn);
    ...
    const dropdown = el("div", { class: "csel-dropdown" });
    options.forEach(o => {
      const item = el("div", { class: "csel-item" + (o.value === selected ? " active" : ""), text: o.label });
      item.addEventListener("click", () => {
        selected = o.value;
        lbl.textContent = o.label;
        dropdown.querySelectorAll(".csel-item").forEach(it => it.classList.remove("active"));
    ...
    saveBtn.addEventListener("click", () => {
      onSave(selected, labelFor(selected));
      J.notify({ kind: "success", text: "Appliqué" });
    });
    const ctrl = el("div", { style: { display: "flex", gap: "8px", alignItems: "center" } });
    ctrl.appendChild(wrap); ctrl.appendChild(saveBtn);
    return ctrl;
  ...
  async function renderPreferences() {
    ...
    musicList.appendChild(settingRow("Fournisseur", "MUSIC_PROVIDER",
      makeSelect(["—", "spotify", "deezer", "local", "jarvis"], music.music_provider || "—", "MUSIC_PROVIDER")));
    musicList.appendChild(el("div", {
      style: { fontSize: "11px", opacity: "0.55", marginTop: "6px" },
      text: "« jarvis » = player natif (lecture directe dans l'onglet). Bibliothèque locale : dépose tes fichiers audio dans le dossier " + (music.music_library_dir || "music_library") + "/ à la racine du projet.",
    }));

    // Sortie audio du player natif Jarvis — préférence purement locale au
    // navigateur (localStorage), pas de clé .env : le choix du périphérique
    // n'a de sens que sur CETTE machine. home.js (document parent, où vit
    // l'élément <audio>) applique setSinkId() en lisant cette même clé.
    const sinkSupported = typeof HTMLMediaElement !== "undefined"
      && "setSinkId" in HTMLMediaElement.prototype;
    if (!sinkSupported) {
      musicList.appendChild(settingRow("Sortie audio (player Jarvis)",
        "Non supporté par ce navigateur.",
        el("div", { style: { fontSize: "12px", opacity: "0.5" }, text: "—" })));
    } else if ("selectAudioOutput" in navigator.mediaDevices) {
      // API dédiée (spec récente) : ouvre le sélecteur natif du navigateur —
      // pas d'énumération à l'aveugle, pas de souci de permission/liste vide.
      const savedLabel = localStorage.getItem("jarvis_audio_sink_label") || "Périphérique par défaut";
      const pickBtn = el("button", { class: "csel-btn" });
      const lbl = el("span", { class: "csel-label", text: savedLabel });
      pickBtn.appendChild(lbl);
      pickBtn.appendChild(el("span", { class: "csel-chevron",
        html: '<svg width="10" height="6" viewBox="0 0 10 6" fill="none"><path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>' }));
      const saveBtn = el("button", { class: "m-btn", text: "Sauv." });
      // Pas de vrai menu déroulant ici (contrairement à Fournisseur) — un
      // seul geste utilisateur ouvre le sélecteur natif du navigateur et
      // vaut à la fois "choix" et "validation". Les deux boutons déclenchent
      // donc la même action, pour que cliquer sur le libellé fonctionne
      // aussi (attendu vu qu'il a le même look que le menu Fournisseur).
      async function pickOutputDevice() {
        try {
          const device = await navigator.mediaDevices.selectAudioOutput();
          localStorage.setItem("jarvis_audio_sink_id", device.deviceId);
          localStorage.setItem("jarvis_audio_sink_label", device.label || "Périphérique choisi");
          lbl.textContent = device.label || "Périphérique choisi";

          // Applique tout de suite, dans le document PARENT (où vit
          // <audio id="jarvis-player">) — cette page (Réglages) tourne dans
          // une iframe séparée, sans accès direct à l'élément. window.parent
          // est accessible (même origine). On applique dans la foulée du
          // choix plutôt que de compter uniquement sur localStorage + un
          // rechargement ultérieur, qui a échoué (DOMException côté Firefox).
          let applied = false;
          try {
            const parentAudio = window.parent?.document?.getElementById("jarvis-player");
            if (parentAudio && typeof parentAudio.setSinkId === "function") {
              await parentAudio.setSinkId(device.deviceId);
              applied = true;
            }
          } catch (err) {
            console.warn("[settings] setSinkId immédiat a échoué", err);
          }

          J.notify({
            kind: applied ? "success" : "info",
            text: applied ? "Sortie audio · appliqué" : "Choix enregistré (sera appliqué à la prochaine lecture).",
          });
        } catch (e) {
          J.notify({ kind: "error", text: "Sélection annulée ou refusée." });
        }
      }
      pickBtn.addEventListener("click", pickOutputDevice);
      saveBtn.addEventListener("click", pickOutputDevice);
      const ctrl = el("div", { style: { display: "flex", gap: "8px", alignItems: "center" } });
      ctrl.appendChild(pickBtn); ctrl.appendChild(saveBtn);
      musicList.appendChild(settingRow("Sortie audio (player Jarvis)",
        "Périphérique pour la lecture native (SoundCloud / bibliothèque locale)", ctrl));
    } else {
      // Fallback : enumerateDevices() ne révèle les vrais périphériques de
      // sortie (labels + entrées) qu'après une permission média accordée sur
      // l'origine — sans ça la liste reste vide à part l'entrée par défaut.
      try {
        const s = await navigator.mediaDevices.getUserMedia({ audio: true });
        s.getTracks().forEach(t => t.stop());
      } catch (_) { /* refusé : liste probablement vide, tant pis */ }
      let devices = [];
      try {
      ...
      const options = [
        { value: "", label: "Périphérique par défaut" },
        ...devices.map(d => ({ value: d.deviceId, label: d.label || ("Périphérique " + d.deviceId.slice(0, 8)) })),
      ];
      const saved = localStorage.getItem("jarvis_audio_sink_id") || "";
      musicList.appendChild(settingRow("Sortie audio (player Jarvis)",
        "Périphérique pour la lecture native (SoundCloud / bibliothèque locale)",
        makeLocalSelect(options, saved, (value, label) => {
          localStorage.setItem("jarvis_audio_sink_id", value);
          localStorage.setItem("jarvis_audio_sink_label", label);
        })));
    }
  ...
  async function renderModeles() {
    ...
    // Chaque backend a sa propre variable de modèle .env + son propre champ
    // dans /api settings ("llm.<field>") — le dropdown doit lire/écrire la
    // bonne paire selon le backend actif, jamais ANTHROPIC_MODEL en dur.
    const MODEL_ENV_BY_BACKEND = {
      anthropic: { key: "ANTHROPIC_MODEL", value: () => llm.anthropic_model },
      mistral:   { key: "MISTRAL_MODEL",   value: () => llm.mistral_model },
      openai:    { key: "OPENAI_MODEL",    value: () => llm.openai_model },
    };
    ...
    function buildApiContent() {
      ...
      function refreshModelRow(backend) {
        ...
        const envInfo = MODEL_ENV_BY_BACKEND[backend] || MODEL_ENV_BY_BACKEND.anthropic;
        modelDiv.innerHTML = "";
        modelDiv.appendChild(settingRow("Modèle", envInfo.key,
          makeSelect(models, envInfo.value(), envInfo.key)));
      }
  ...
  async function renderApropos() {
    ...
    // ── Bouton mise à jour ────────────────────────────────────
    // Flux en deux temps : 1) vérifier (fetch seul, rien n'est appliqué)
    // 2) appliquer, seulement si l'utilisateur reclique après avoir vu qu'une
    // mise à jour est disponible. Jamais d'application automatique.
    ...
    let updatePhase = "check"; // "check" | "apply" | "restart"
    ...
    updateBtn.addEventListener("click", async () => {
      ...
      if (updatePhase === "check") {
        updateBtn.textContent = "Vérification…";
        try {
          const res = await J.api.post("/admin/api/system/check-update", {});
          if (!res.ok) {
            updateStatus.textContent = "Erreur : " + (res.error || "Vérification impossible.");
            ...
            updateBtn.textContent = "Vérifier les mises à jour";
          } else {
            const n = res.commits_behind;
            updateStatus.textContent = `Mise à jour disponible (${n} commit${n > 1 ? "s" : ""}). Reclique pour l'appliquer.`;
            updateStatus.className = "about2-update-status ok";
            updateBtn.textContent = "Appliquer la mise à jour";
            updatePhase = "apply";
          }
        } catch (_) {
      ...
      // updatePhase === "apply" : appliqué uniquement sur ce second clic explicite
      ...
      try {
        ...
        if (res.already_up_to_date) {
          ...
          let msg = "Erreur : " + (res.error || "Échec de la mise à jour.");
          if (res.conflicted_files && res.conflicted_files.length) {
            msg += "\nFichiers en conflit : " + res.conflicted_files.join(", ");
          }
