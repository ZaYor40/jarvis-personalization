...
(function () {
    ...
    // Cache module (durée de vie = la page) : recréer le FaceLandmarker à
    // chaque scan (WASM + contexte GL + graphe TFLite) provoque un vrai freeze
    // visible à chaque fois. On l'initialise une seule fois et on le réutilise
    // pour tous les scans suivants — cleanup() ne le ferme donc plus jamais.
    let _cachedLandmarker = null;
    let _landmarkerInitPromise = null;

    // Détection de vivacité (anti-spoofing léger) : exige un clignement des
    // yeux réel pendant le scan avant de valider — une photo/écran ne cligne
    // pas des yeux. Seuils sur les blendshapes MediaPipe eyeBlinkLeft/Right
    // (0 = ouvert, 1 = fermé).
    const BLINK_CLOSE_THRESHOLD = 0.5;
    const BLINK_OPEN_THRESHOLD  = 0.2;
    const LIVENESS_TIMEOUT_MS   = 7000; // délai max pour observer un clignement
    ...
    function runFacePhase(rootEl, opts, onDone) {
        ...
        const requireLiveness = !!opts.requireLiveness;
        // Si la caméra échoue (permission refusée, périphérique occupé...), le
        // scan bascule en mode simulation et valide normalement par défaut
        // (CDC §C4 : jamais bloquant). Mais un appelant qui a explicitement
        // demandé un vrai verrou (blockOnUnrecognized) ne doit PAS être
        // contournable simplement en empêchant la caméra de démarrer — dans ce
        // cas la simulation doit refuser, pas valider en silence.
        const blockOnUnrecognized = !!opts.blockOnUnrecognized;
        ...
        let blinkDetected  = false; // passe à true dès qu'un cycle fermé→ouvert est observé
        let cleanedUp    = false;
        let raf          = 0;
            ...
            try {
                ...
                try {
                    if (_cachedLandmarker) {
                        // Réutilisation : pas de re-fetch WASM/modèle, pas de nouveau
                        // contexte GL — élimine le freeze visible à chaque scan.
                        landmarker = _cachedLandmarker;
                    } else {
                        if (!_landmarkerInitPromise) {
                            _landmarkerInitPromise = (async function () {
                                ...
                                return vision.FaceLandmarker.createFromOptions(fileset, {
                                    ...
                                    runningMode: 'VIDEO',
                                    numFaces: 1,
                                    // Blendshapes : source du signal de clignement (eyeBlinkLeft/Right)
                                    // pour la détection de vivacité (requireLiveness).
                                    outputFaceBlendshapes: true,
                                });
                            })();
                        }
                        landmarker = await _landmarkerInitPromise;
                        _cachedLandmarker = landmarker;
                    }
                } catch (e) {
                    console.warn('[wake][face] MediaPipe non disponible — vidéo brute sans landmarks', e);
                    landmarker = null;
                    _landmarkerInitPromise = null; // retente une init propre au prochain scan
                }
                if (disposed) return;
        ...
        function tick() {
            ...
            if (!simulationMode && landmarker && video.readyState >= 2) {
                try {
                    ...
                    if (requireLiveness && !blinkDetected && r && r.faceBlendshapes && r.faceBlendshapes[0]) {
                        ...
                        const closedness = (left + right) / 2;
                        if (closedness > BLINK_CLOSE_THRESHOLD) {
                            eyesWereClosed = true;
                        } else if (eyesWereClosed && closedness < BLINK_OPEN_THRESHOLD) {
                            blinkDetected = true; // cycle fermé → ouvert complet = un clignement réel
                        }
                    }
                } catch (e) {}
            }
        ...
        function maybeShowVerdict() {
            ...
            // Vivacité requise : on laisse une fenêtre pour observer un clignement
            // naturel/volontaire avant de trancher (une photo ne cligne jamais).
            if (requireLiveness && !simulationMode && !blinkDetected
                && scanStart !== null && (performance.now() - scanStart) < LIVENESS_TIMEOUT_MS) {
                status.textContent = 'CLIGNE DES YEUX POUR CONFIRMER…';
                return;
            }
            verdictShown = true;

            // Verdict positif si :
            //   - mode simulation ET pas de blocage strict demandé (CDC §C4 : jamais
            //     bloquant par défaut — mais un appelant strict refuse la simulation,
            //     sinon il suffirait de bloquer la caméra pour contourner le verrou), ou
            //   - backend a confirmé recognized=true, ou
            //   - mode live mais pas assez de frames détectées → dérogation (CDC §C4)
            let recognized = false;
            if (simulationMode) {
                recognized = !blockOnUnrecognized;
            } else if (backendResult && backendResult.recognized) {
                recognized = true;
            }

            // Vivacité : un identity-match sans clignement observé reste refusé
            // (photo/écran plausible). N'affecte que les appelants qui l'exigent
            // explicitement (opts.requireLiveness) — comportement par défaut inchangé.
            const livenessFailed = requireLiveness && !simulationMode && !blinkDetected;
            if (livenessFailed) recognized = false;
            ...
            if (recognized) {
                ...
                // CDC §C4 : par défaut on ne bloque jamais localement (le caller
                // décide via opts.blockOnUnrecognized / opts.onFailure). Message
                // dérogation, distinct si c'est spécifiquement la vivacité qui a échoué
                // (identité OK mais aucun clignement observé — photo/écran suspecté).
                status.textContent = simulationMode
                    ? 'CAMÉRA INDISPONIBLE · ACCÈS REFUSÉ'
                    : livenessFailed
                        ? 'VIVACITÉ NON CONFIRMÉE · AUCUN CLIGNEMENT'
                        : 'VISAGE NON RECONNU · DÉROGATION MANUELLE';
        ...
        function cleanup() {
            ...
            // landmarker n'est JAMAIS fermé ici : c'est l'instance mise en cache
            // (_cachedLandmarker) partagée entre tous les scans de la page.
    ...
    function createWakeSequence(rootEl, opts) {
        ...
        // onFailure : hors CDC C4 (qui ne bloque jamais l'accès par défaut — cf. plus
        // bas). Uniquement appelé si blockOnUnrecognized est explicitement demandé par
        // l'appelant (ex. réveil manuel post-boot, où un vrai blocage est souhaité).
        const onFailure          = (typeof opts.onFailure === 'function') ? opts.onFailure : function () {};
        const blockOnUnrecognized = !!opts.blockOnUnrecognized;
        const userName       = opts.userName    || '';
            ...
            switch (s) {
                ...
                case STATES.FACE:
                    ...
                    faceHandle = runFacePhase(rootEl, {
                        ...
                        requireLiveness: opts.requireLiveness,
                        blockOnUnrecognized: blockOnUnrecognized,
                    }, function (recognized) {
                        faceHandle = null;
                        if (!recognized && blockOnUnrecognized) {
                            // Blocage explicitement demandé par l'appelant (opts.blockOnUnrecognized) :
                            // le verdict rouge a déjà été montré côté UI (runFacePhase). On ne
                            // continue PAS vers le dashboard — on notifie l'appelant, qui décide
                            // (ex. Mode veille : retour à l'écran STANDBY, nouvel essai possible).
                            try { onFailure(); } catch (e) { console.error('[wake] onFailure error', e); }
                            return;
                        }
                        // Cas par défaut (CDC C4, cold boot inclus) : on ne bloque jamais.
                        // Un visage non reconnu -- ou une erreur backend (pas de cv2/modèle,
                        // timeout, visage non enrôlé) -- a déjà montré le verdict de dérogation
                        // manuelle. On avance quand même vers le dashboard : sinon l'utilisateur
                        // reste coincé sur l'écran de veille, micro hors d'atteinte.
    ...
    function createSleepScreen(rootEl, opts) {
        ...
        let triggered = false;
        // onWake(source) : 'clap' (déclenché à distance, potentiellement par
        // n'importe qui à portée de voix) vs 'click'/'keydown' (accès physique
        // direct au clavier/souris — déjà une preuve de présence). Le caller
        // décide quoi faire de chaque source (ex. n'exiger la vérification
        // faciale que pour 'clap').
        function handleWake(source) {
            if (triggered) return;
            triggered = true;
            onWake(source);
        }
        window.addEventListener('jarvis-wake', function (e) {
            handleWake((e && e.detail && e.detail.trigger) || 'jarvis-wake');
        });
        ...
        rootEl.addEventListener('click', function () { handleWake('click'); });
        window.addEventListener('keydown', function () { handleWake('keydown'); });
